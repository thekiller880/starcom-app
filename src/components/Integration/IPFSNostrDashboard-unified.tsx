/**
 * IPFS-Nostr Integration Dashboard Component (Unified Version)
 * 
 ...
 * 
 * Uses the unified chat context for Nostr communication aspects.
 */
...
export const IPFSNostrDashboard: React.FC<DashboardProps> = ({ teamId, userId }) => {
  ...
  // Initialize chat connection and monitoring channel
  useEffect(() => {
    const initializeNostrChannel = async () => {
      try {
        if (!chat.isConnected) {
          // Connect to chat service if not already connected
          await chat.connect({
            type: 'nostr', // Use Nostr for IPFS-Nostr integration
            options: {
              userId,
        ...
        // Create or join the IPFS-Nostr monitoring channel
        const monitoringChannelId = `ipfs-nostr-${teamId}`;
        
        // Check if the channel exists
        if (!chat.channels.some(channel => channel.id === monitoringChannelId)) {
          // Create the channel if it doesn't exist
          await chat.createChannel(
            `IPFS-Nostr Integration (${teamId})`,
            'team',
            [userId]
          );
        }
        ...
        // Join the channel
        await chat.joinChannel(monitoringChannelId);
        
        // Set as current channel
        chat.setCurrentChannel(monitoringChannelId);
        
        setChannelInitialized(true);
      } catch (error) {
        console.error('Failed to initialize Nostr channel:', error);
      }
    };
    
    initializeNostrChannel();
  }, [chat, teamId, userId]);

  // Convert chat messages to activity events if they contain IPFS content
  const messageToActivityEvent = useCallback((msg: ChatMessage): ActivityEvent | null => {
    if (!msg.content.includes('IPFS') && !msg.content.includes('ipfs:')) {
      return null;
    }
    
    // Parse IPFS hash from message if present
    const ipfsHashMatch = msg.content.match(/ipfs:([a-zA-Z0-9]{46})/);
    const ipfsHash = ipfsHashMatch ? ipfsHashMatch[1] : undefined;
    
    return {
      id: msg.id,
      timestamp: new Date(msg.timestamp),
      type: msg.content.includes('updated') ? 'content-updated' : 'content-stored',
  ...
  // Monitor chat messages for IPFS-related events
  useEffect(() => {
    if (chat.isConnected && chat.currentChannel && channelInitialized) {
      // Process new messages as activity events
      const currentMessages = chat.messages[chat.currentChannel] || [];
      
      // Convert chat messages to activity events if they contain IPFS content
      const ipfsRelatedEvents = currentMessages
        .map(messageToActivityEvent)
        .filter(Boolean) as ActivityEvent[];
      
      if (ipfsRelatedEvents.length > 0) {
        setActivityFeed(prev => {
          // Combine with existing events and remove duplicates
          const combined = [...ipfsRelatedEvents, ...prev];
  ...
  }, [chat.isConnected, chat.currentChannel, chat.messages, channelInitialized, messageToActivityEvent]);
  ...
  const updateNetworkStatus = async () => {
    try {
      ...
      // Enhance with chat connection status
      status.nostr.connected = chat.isConnected;
      status.nostr.relayCount = chat.channels.length;
  ...
  const loadActivityFeed = () => {
    ...
    setActivityFeed(prev => {
      // Combine with existing events and remove duplicates
      const combined = [...mockActivity, ...prev];
      const uniqueEvents = combined.filter((event, index, self) => 
        index === self.findIndex(e => e.id === event.id)
      );
      return uniqueEvents.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime());
  ...
  const handleContentStored = (event: any) => {
    ...
    // Announce in chat channel if connected
    if (chat.isConnected && chat.currentChannel) {
      chat.sendMessage(`[IPFS Content Stored] ${event.content.title} (ipfs:${event.content.ipfsHash})`).catch(err => {
        console.error('Failed to announce content in chat:', err);
      });
    }
  };
  ...
  const handleContentUpdated = (event: any) => {
    ...
    // Announce in chat channel if connected
    if (chat.isConnected && chat.currentChannel) {
      chat.sendMessage(`[IPFS Content Updated] ${event.contentId} to v${event.version} (ipfs:${event.newHash})`).catch(err => {
        console.error('Failed to announce update in chat:', err);
      });
    }
  };
  ...
  const handleCollaboration = (event: any) => {
    ...
    // Announce in chat channel if connected
    if (chat.isConnected && chat.currentChannel) {
      chat.sendMessage(`[Collaboration] ${event.userId} - ${event.type} on ${event.contentId}`).catch(err => {
        console.error('Failed to announce collaboration in chat:', err);
      });
    }
  };
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Also send a chat message to query other peers
      if (chat.isConnected && chat.currentChannel) {
        await chat.sendMessage(`[SEARCH] ${searchQuery}`);
      }
  ...
  const handleStoreContent = async (contentData: IntelPackage | CyberTeam | CyberInvestigation) => {
    try {
      const result = await unifiedService.storeContent(contentData, {
        teamId,
        creator: userId,
      ...
      // Announce in chat
      if (chat.isConnected && chat.currentChannel) {
        await chat.sendMessage(`[IPFS Content Created] ${result.title} (ipfs:${result.ipfsHash})`);
      }
    ...
    <div className="ipfs-nostr-dashboard bg-gray-900 text-white p-6 rounded-lg">
      ...
      <div className="mt-6 pt-4 border-t border-gray-700 flex justify-between items-center text-sm">
        <div className="flex items-center space-x-2">
          <span className={`w-2 h-2 rounded-full ${chat.isConnected ? 'bg-green-500' : 'bg-red-500'}`}></span>
          <span>{chat.isConnected ? 'Connected to Nostr' : 'Disconnected'}</span>
        </div>
        <div className="text-gray-500">
          Using {chat.providerType} provider | Encryption: {chat.isEncryptionEnabled ? 'Enabled' : 'Disabled'}
        </div>
      </div>
    </div>
  );
